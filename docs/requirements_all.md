...
* **US-7:** 私は、ステーションの在庫が時間経過で**「腐敗・劣化」**し、常に需要が発生し続ける「生きた経済」の中で活動したい。
...
* **US-13:** システムは、ステーション在庫の自然劣化（腐敗）をシミュレートし、常に新規生産と物流が必要な状態を維持したい。
...
* **腐敗（Decay）システム:**
    * 各在庫品目ごとの減衰関数実装（食料は早く、鉱石は遅く劣化）。
    * Tickベースでの在庫更新処理。
* **生産シミュレーション:**
    * 各ステーションが一定時間ごとに一定量の物資を生産する。
    * 生産には材料が必要。なければ生産されない
    * 生産物は搬出されなければ、ステーション在庫として腐敗していく
